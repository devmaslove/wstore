...
Хочется чтобы снаружи наш виджет оставался всё темже виджетом с параметрами/калбеками.
...
При изменении данных вызываем у сторы `setStore` - уведоиляем билдеры
...
подождать. Только после того как поступят в оба нужно нужно данные как-то преобразовать в единое
целое. Для упрощения такой обработки сделана фунция `subscribe2`
...
Значение попадет в `onData` только после того когда оба потока получили хотябы по одному
элементу, дальше туда будут передаваться посление данные при любом обновлении любого потока.
