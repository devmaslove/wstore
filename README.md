...
Если нужно добавить вычисляемый геттер в стору, то его нужно обернуть в метод
`compose`, тогда билдеры будут обновляться только когда пересчитается значение.
