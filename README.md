...
обновляться только когда пересчитается исходное значение в списке `watch`
(`compose` вычисляет значение один раз по `getValue` и кеширует его ключу `keyName`,
а сбрасывается кэш при обновлении переменных списка `watch`).
